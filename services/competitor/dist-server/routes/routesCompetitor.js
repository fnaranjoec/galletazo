...
var _competitor = require("../src/db/models/competitor");

var _competitor_tried = require("../src/db/models/competitor_tried");

var _prize = require("../src/db/models/prize");

var _promocode = require("../src/db/models/promocode");

var _vw_winners = require("../src/db/models/vw_winners");

var _competitor_tried_view = require("../src/db/models/competitor_tried_view");
...
const CircularJSON = require('circular-json'); // ---------------------------------------------------- COMPETITOR --------------------------------------------------------------
// *** FETCH ALL
...
router.post(`/all`, async (req, res, next) => {
  ...
  try {
    const competitors = await _competitor.CompetitorModel.findAndCountAll({
...
router.get(`/:competitor_id`, async (req, res, next) => {
  try {
    var competitor = await _competitor.CompetitorModel.findOne({
      attributes: {},
      where: {
...
router.get(`/external/:competitor_external_id`, async (req, res, next) => {
  ...
  try {
    var competitor = await _competitor.CompetitorModel.findOne({
      attributes: {},
      where: {
        competitor_external_id: req.params.competitor_external_id // [sequelize.Op.not] : [{competitor_status: "X"}]
...
router.post(`/`, async (req, res, next) => {
  ...
  if (!req.body.args.competitor_firstname || !req.body.args.competitor_lastname || !req.body.args.competitor_external_id) {
  ...
  try {
    ...
    newJSON['competitor_id'] = (0, _generateUUID.default)();
    ...
    if (req.body.args.competitor_external_id) newJSON['competitor_external_id'] = req.body.args.competitor_external_id; // console.log("newJSON=>", newJSON);

    if ((0, _isEmpty.default)(newJSON)) {
      return next(new Error("Nothing to create !"));
    }

    const newCompetitor = await _competitor.CompetitorModel.create(newJSON); // console.log("newCompetitor=>", newCompetitor);
...
router.put(`/:competitor_external_id`, async (req, res, next) => {
  ...
  try {
    const [numberOfAffectedRows, affectedRows] = await _competitor.CompetitorModel.update(updateJSON, {
      where: {
    ...
    const updatedCompetitor = await _competitor.CompetitorModel.findOne({
      attributes: {},
      where: {
        competitor_external_id: req.params.competitor_external_id,
        [sequelize.Op.not]: [{
...
router.delete(`/:competitor_id`, async (req, res, next) => {
  ...
  const deletedCompetitor = await _competitor.CompetitorModel.findOne({
    attributes: {},
    where: {
      competitor_id: req.params.competitor_id,
      [sequelize.Op.not]: [{
  ...
  try {
    ...
    // Borrado fisico
    await deletedCompetitor.destroy(); // return res.end();
...
router.get(`/tried/v1/:competitor_external_id/:promocode_code`, async (req, res, next) => {
  ...
  async function getStartCampaignDate(parameter_name = 'START_CAMPAIGN') {
    try {
      ...
      return (await _parameter.ParameterModel.findOne({
        raw: true,
        attributes: {},
        where: {
          parameter_name: {
            [sequelize.Op.eq]: parameter_name
          }
        }
      })).parameter_text || "";
    } catch (err) {
  ...
  async function getPrizeDefault(idPremioDefault) {
    try {
      ...
      return await _prize.PrizeModel.findOne({
        raw: true,
        attributes: {},
        where: {
          prize_id: {
            [sequelize.Op.eq]: idPremioDefault
          }
        }
      });
    } catch (err) {
  ...
  async function isValidCompetitor(competitor_external_id) {
    try {
      ...
      return await _competitor.CompetitorModel.findOne({
        attributes: {},
        where: {
  ...
  async function isValidPromocode(promocode_code) {
    try {
      ...
      return await _promocode.PromocodeModel.findOne({
        raw: true,
        attributes: {},
        where: {
          [sequelize.Op.and]: {
            promocode_code: {
              [sequelize.Op.eq]: promocode_code
            },
            promocode_status: {
              [sequelize.Op.notIn]: ["X"]
            }
          }
        }
      });
  ...
  async function markPromocodeAsUsed(promocode_code) {
    try {
      ...
      const [numberOfAffectedRows2, affectedRows2] = await _promocode.PromocodeModel.update(updateJSON, {
        where: {
          promocode_code: {
            [sequelize.Op.eq]: promocode_code
          }
  ...
  async function isWinnerBefore(competitor_id) {
    try {
      ...
      return await _vw_winners.VwWinnersModel.findOne({
        attributes: {},
        where: {
  ...
  async function updateInvetory(inventarioPremios, parameter_name = 'DAYRI_PRIZE') {
    try {
      ...
      const [numberOfAffectedRows, affectedRows] = await _parameter.ParameterModel.update({
  ...
  if (!prizeDefault) return next(new Error("Premio default no existe !")); // Verifico competidor

  var competitor = await isValidCompetitor(req.params.competitor_external_id);
  if (!competitor) return next(new Error("Competidor no existe !")); // Verifico promocode

  var promocode = await isValidPromocode(req.params.promocode_code);
  if (!promocode) return next(new Error("Codigo no existe o ya fue usado !")); // Marco promocode como usado

  await markPromocodeAsUsed(req.params.promocode_code); // Verifico si gano antes

  var winner = await isWinnerBefore(competitor.competitor_id);

  if (!(0, _isEmpty.default)(winner) || winner != null) {
    return res.json({
      gano: false,
      premio: prize
    });
  ...
  if (!limitProbability) return next(new Error("Parametro LIMIT_PROBABILITY no existe !")); // Obtengo numero aleatorio

  var triedNumber = parseInt(Math.random() * 100); // Obtengo estado de comparacion (Gano o perdio)

  console.log(`Tirada: ${triedNumber}, Probabiliad: ${limitProbability}`);
  var triedStatus = triedNumber > limitProbability; // To save later on competitor_tried table
  ...
  if (triedStatus) {
    ...
    competitorTriedJSON['prize_id'] = prizeDefault.prize_id; // Grabo el competitors_tried

    var newCompetitorTried = await _competitor_tried.CompetitorTriedModel.create(competitorTriedJSON);
    if (!newCompetitorTried) return next(new Error("Error al grabar nuevo Competitor Tried !")); //retorno con datos del premio
...
router.get(`/tried/v2/:competitor_external_id/:promocode_code`, async (req, res, next) => {
  ...
  async function availableQtyPrizes() {
    try {
      ...
      return (await _prize.PrizeModel.findAndCountAll({
        raw: true,
        attributes: {},
        where: {
          [sequelize.Op.and]: {
            prize_qty: {
              [sequelize.Op.lt]: sequelize.Sequelize.col('prize_period_qty')
            },
            prize_status: {
              [sequelize.Op.notIn]: ['X']
            }
  ...
  async function isValidTimePrizeRequest(periodType, periodValue, lastDelivery, currDate) {
    try {
      var timeDiff; // Si lastDelivery es null

      lastDelivery = lastDelivery || new Date(-8640000000000000); // Comparo la diferencia entre el momento actual y el ultimo delivery
  ...
  async function availableTimePrizes(array) {
    ...
    while (i < len) {
      ...
      var isValidPrizeRequest = await isValidTimePrizeRequest(array[i].prize_period_type, array[i].prize_period_value, array[i].prize_last_delivery, currDate); // console.log(`isValidPrizeRequest-->${JSON.stringify(isValidPrizeRequest)}`);

      if (isValidPrizeRequest.message && array[i].prize_period_qty - array[i].prize_qty > 0) {
        newArray.push(array[i]);
      } // else {
  ...
  async function updatePrizeQty(id) {
    try {
      ...
      const [numberOfAffectedRows3, affectedRows3] = await _prize.PrizeModel.update({
        prize_qty: sequelize.Sequelize.literal('prize_qty + 1'),
  ...
  async function getTriedsCompetitor(id) {
    try {
      ...
      return (await _connectiondb.default.query('SELECT competitor_id, count(competitor_tried_id) as competitor_count FROM competitor_tried WHERE competitor_id = $competitor_id GROUP BY competitor_id ', {
        bind: {
          competitor_id: id
        },
  ...
  var prize = {}; // Verifico competidor

  var competitor = await isValidCompetitor(req.params.competitor_external_id);
  if (!competitor) return next(new Error("Competidor no existe !")); // Verifico promocode

  var promocode = await isValidPromocode(req.params.promocode_code); // console.log(`promocode-->${JSON.stringify(promocode)}`);

  if (!promocode) return next(new Error("Código no existe o ya fue usado !")); // Marco Promocode como usado

  await markPromocodeAsUsed(req.params.promocode_code); // To save later on competitor_tried table

  var competitorTriedJSON = {
    promocode_id: promocode.promocode_id,
    competitor_id: competitor.competitor_id
  };
  var newCompetitorTried; // Verifico cantidad de premios disponibles

  const prizeQtyArray = await availableQtyPrizes();

  if (!prizeQtyArray || prizeQtyArray.length == 0) {
    newCompetitorTried = await _competitor_tried.CompetitorTriedModel.create(competitorTriedJSON);
    return res.json({
      gano: false,
      premio: prize
    });
  ...
  const availableArrayPrize = await availableTimePrizes(prizeQtyArray); // console.log(`availableArrayPrize-->${JSON.stringify(availableArrayPrize)}`);
  ...
  var limitProbability = await getLimitProbability();
  if (!limitProbability) return next(new Error("Parametro LIMIT_PROBABILITY no existe !")); // comparo frecuencia para ver si cambia la probabilidad
  ...
  console.log(`triedsCompetitor=${triedsCompetitor}, limitToFrequent=${limitToFrequent}`); // Si las jugadas son mayores al parametro se lo considera COMPETIDOR FRECUENTE
  ...
  var triedStatus = triedNumber > limitProbability; // Proceso segun estado de comparacion
  // --------------------------------------------------------------- GANO

  if (triedStatus) {
    //Busco premio aleatoriamente en array de premios disponibles
    var indexPrize = Math.floor(Math.random() * (availableArrayPrize.length + 1 - 1) + 1) - 1; // Actualizo QTY en tabla de premios
    ...
    competitorTriedJSON['prize_id'] = availableArrayPrize[indexPrize].prize_id; // Grabo el competitors_tried
...
router.get(`/tried/view/count`, async (req, res, next) => {
  try {
    const competitors_tried = await _competitor_tried_view.CompetitorTriedViewModel.findAndCountAll();
...
router.post(`/tried/view`, async (req, res, next) => {
  console.log(`req.body.args-->${JSON.stringify(req.body.args)}`); // var filter={};
  // if ('filter' in req.body.args) {
  //     console.log('encontro filter');
  //     filter = await getWhere({filter: req.body.args.filter, status: ""});
  // }

  const filter = 'filter' in req.body.args ? await (0, _getWhere.default)({
    filter: req.body.args.filter,
    status: ""
  }) : {};
