...
const ENDPOINT_URI = (0, _accessEnv.default)("ENDPOINT_URI"); // const BASE_URL_INTERCOM = accessEnv("BASE_URL_INTERCOM");
// const TOKEN_INTERCOM = accessEnv("TOKEN_INTERCOM");

const CircularJSON = require('circular-json'); // ---------------------------------------------------- USER --------------------------------------------------------------
// *** FETCH ALL USER
...
router.post(`/all`, async (req, res, next) => {
  ...
  try {
    const users = await _models.UserModel.findAndCountAll({
...
router.post(`/rank/all`, async (req, res, next) => {
  ...
  try {
    const users = await _models.UserRankModel.findAndCountAll({
      // order: [['user_created','desc']],
      attributes: {},
      where: filter
    });
...
router.get(`/:user_id`, async (req, res, next) => {
  try {
    var user = await _models.UserModel.findOne({
      attributes: {},
      where: {
...
router.get(`/rank/:user_email`, async (req, res, next) => {
  try {
    var user = await _models.UserRankModel.findOne({
      attributes: {},
      where: {
        user_email: req.params.user_email // [sequelize.Op.not] : [{user_status: "X"}]
...
router.post(`/`, async (req, res, next) => {
  ...
  try {
    ...
    const oldUser = await _models.UserModel.findOne({
      attributes: {},
      where: {
    ...
    if (req.body.args.referencial_id) {
      const referencialUser = await _models.UserModel.findOne({
        attributes: {},
        where: {
    ...
    var newJSON = {};
    newJSON['user_id'] = (0, _generateUUID.default)();
    ...
    const newUser = await _models.UserModel.create(newJSON); // Creo ROW USUARIO para ser guardado
    // Si hay codigo de referido lo grabo en USER_REFERENCIAL

    if (req.body.args.referencial_id) {
      const newUserReferencial = await _models.UserReferencialModel.create({
        user_referencial_id: (0, _generateUUID.default)(),
        user_id: req.body.args.referencial_id,
...
router.put(`/:user_id`, async (req, res, next) => {
  ...
  try {
    const [numberOfAffectedRows, affectedRows] = await _models.UserModel.update(updateJSON, {
      where: {
        user_id: req.params.user_id,
        [sequelize.Op.not]: [{
          user_status: "X"
    ...
    const updatedUser = await _models.UserModel.findOne({
      attributes: {},
      where: {
        user_id: req.params.user_id,
        [sequelize.Op.not]: [{
...
router.delete(`/:user_id`, async (req, res, next) => {
  ...
  const deletedUser = await _models.UserModel.findOne({
    attributes: {},
    where: {
      user_id: req.params.user_id,
      [sequelize.Op.not]: [{
  ...
  if (!deletedUser) return next(new Error("Invalid User ID or Deleted")); // VERIFICO QUE NO HAYA USUARIO_REFENCIALES GENERADOS

  const userReferencials = await _models.UserReferencialModel.findAll({
    attributes: {},
    where: {
      user_id: req.params.user_id // [sequelize.Op.not] : [{user_referencial_status: "X"}]

    }
  });
  if (!(0, _isEmpty.default)(userReferencials)) return next(new Error("User has referencial users, can't delete it !!!"));
  ...
  try {
    ...
    // Borrado fisico
    await deletedUser.destroy(); // return res.end();
