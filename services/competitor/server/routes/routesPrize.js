...
const CircularJSON = require('circular-json');
    ...
    router.get( `/reset` , async (req, res, next) => {
        ...
        try {
            var resetMaxPrize = await ParameterModel.findOne({
                attributes: {},
                where: {
                    parameter_name: {[sequelize.Op.eq]:'RESET_MAX_PRIZE'}
                }
            });
    
            var updateJSON = {};
            updateJSON['parameter_value'] = resetMaxPrize.parameter_value;
            ...
            try {
                const [numberOfAffectedRows, affectedRows] = await ParameterModel.update(updateJSON,
                    {
                        where: {
                            parameter_name: {[sequelize.Op.eq]:'DAYRI_PRIZE'}
                        },
                        returning: true,
                    });
